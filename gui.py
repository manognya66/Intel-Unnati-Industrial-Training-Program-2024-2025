...
def execute_operation():
    ...
    if not input_file or not passphrase:
        ...
        print("Error: Input file and passphrase must be provided!")
        return
    ...
    try:
        if operation_var.get() == "Encrypt":
            ...
            # Show dialog first, then print to console
...
def execute_brute_force():
    ...
    if not input_file or not charset or not max_length:
        ...
        print("Error: Input file, charset, and max length must be provided!")
        return

    try:
        # Insert the initial message (keep it) and ensure view is at top initially
        debug_text.config(state=tk.NORMAL)
        debug_text.delete(1.0, tk.END)
        debug_text.insert(tk.END, "Brute-force operation started...\n")
        debug_text.config(state=tk.DISABLED)

        # Show messagebox first (blocks until user closes)
        messagebox.showinfo("Info", "Brute-force operation started. This may take some time.")
        print("Brute-force operation started... This may take some time.")

        # callback appends each attempt on a new line and autoscrolls
        def update_brute_force_text(current_passphrase):
            debug_text.config(state=tk.NORMAL)
            debug_text.insert(tk.END, f"Trying passphrase: {current_passphrase}\n")
            debug_text.see(tk.END)                # auto-scroll to the newest line
            debug_text.config(state=tk.DISABLED)
            print(f"Trying passphrase: {current_passphrase}")
        ...
        if passphrase:
            ...
            print(f"Passphrase found: {passphrase} -- File decrypted successfully.")
            # optional final message in debug box
            debug_text.config(state=tk.NORMAL)
            debug_text.insert(tk.END, f"Passphrase found: {passphrase}\n")
            debug_text.see(tk.END)
            debug_text.config(state=tk.DISABLED)
        else:
            messagebox.showerror("Error", "Brute-force attempt failed to find the passphrase!")
            print("Brute-force attempt failed to find the passphrase!")
            debug_text.config(state=tk.NORMAL)
            debug_text.insert(tk.END, "Brute-force finished: passphrase not found.\n")
            debug_text.see(tk.END)
            debug_text.config(state=tk.DISABLED)

    except Exception as e:
        messagebox.showerror("Error", f"Brute-force operation failed: {str(e)}")
        print(f"Brute-force operation failed: {e}")
        debug_text.config(state=tk.NORMAL)
        debug_text.insert(tk.END, f"Error: {e}\n")
        debug_text.see(tk.END)
        debug_text.config(state=tk.DISABLED)

# ------------------------------
# UI Update Function (previously missing)
# ------------------------------
def update_ui():
    """Update the UI when switching between Encrypt and Decrypt."""
